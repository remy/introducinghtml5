...
> As this secondedition goes to press, the Working Group is discussing removing `<time>` from HTML and, to replace it, magicking up a more generic--and therefore less useful--`<data>` element. We hope that this won't happen but before you use it, please see [introducinghtml5.com](http://www.introducinghtml5.com) to find any errata to this book. Or look at the spec!
...
1. The Monotonous Times 1. Breaking news
...
However, inside <nav> and <aside> it's perfectly legitimate not to have a heading. You may want to do it for some <nav> blocks,
...
such as "Most popular posts" or "Recent comments," but you probably don't want a heading on your main site <nav> that just says "Navigation."
...
Sometimes you have a heading and a subheading, or tagline. Slashdot uses "News for nerds, stuff that matters"; dev.opera has "Follow the Standards. Break the Rules"; Metafilter is a "community weblog." How do you mark up those taglines?

In HTML 4, you could use

<h1>Metafilter</h1> <p>community weblog</p>
...
<h1>Metafilter</h1> <h2>community weblog</h2>
...
1. Unicorns and butterflies 1. Main nav
...
*:heading(1) {font-size: 2.5em;} /* a logical <h1> */ *:heading(2) {font-size: 2em;} /* a logical <h2> */

However, at the time of this writing, this is but a wonderful dream. As a stopgap, Mozilla is experimenting with a new selector grouping mechanism in Firefox nightlies called :-moz-any() that allows a form of CSS shorthand--go to[ http://hacks.mozilla]( http://hacks.mozilla). org/2010/05/moz-any-selector-grouping/ to learn more.
...
A recent survey by __WebAIM__ showed that 57 percent of screen reader users use a site's headings structure as their first method to find information on a lengthy site. (See the full survey for more vital information at[ www.webaim.org/projects/screenreadersurvey3]( www.webaim.org/projects/screenreadersurvey3)). These people use the hierarchy of headings both to give themselves a mental overview (an outline!) of the document they're in and also to navigate through that content. Most screen readers

> Watch the excellentvideo entitled "Importance of HTML Headings for Accessibility" (www.youtube.com/ watch?v=__AmUPhEVWu_E__). The video shows how a blind accessibility consultant navigates a page with JAWS. In an ideal world, it would be compulsory to watch and understand this video before you're allowed to call yourself a professional designer or developer. Sadly, it's not an ideal world.

have keyboard shortcuts that allow users to jump from heading to heading. For example, the JAWS screen reader (by far the most used package, according to the survey) uses the H key to jump from heading to heading, the 1 key to jump to the next <h1>, the 2 key to go to the next <h2>, and so on.
...
> A <section>generally begins with a heading that introduces it. An exception to this might be a <section> that will have
...
is intended to be independently distributable or reusable." It's either a way of sectioning a page into different subject areas, or sectioning an article into, well, sections.
...
> All of your which-structural-element-should-Ichoose conundrums are easily solved with "The Amazing HTML5 Doctor Easily Confused HTML5 Element Flowchart
...
> I've included asearch form inside the <nav> (but outside the <ul>s); it seems appropriate to me to regard a search form as a navigational aid. I've also given it the ARIA role appropriate to its function.
...
> If you start usingthese new ARIA attributes, you may notice that your
